...
func spawn_pieces():
	...
	for i in range(width):
		for j in range(height):
			var rand = rng.randi_range(0, possible_pieces.size() - 1)
			var piece = possible_pieces[rand].instantiate()
			var max_loops = 100
			var loops = 0
			while (match_at(i, j, piece.color) and loops < max_loops):
				rand = rng.randi_range(0, possible_pieces.size() - 1)
				loops += 1
...
func swap_pieces(column, row, direction: Vector2):
	...
	var other_piece = all_pieces[int(column + direction.x)][int(row + direction.y)]
	if first_piece == null or other_piece == null:
		return
	state = WAIT
	store_info(first_piece, other_piece, Vector2(column, row), direction)
	all_pieces[column][row] = other_piece
	all_pieces[int(column + direction.x)][int(row + direction.y)] = first_piece
	first_piece.move(grid_to_pixel(int(column + direction.x), int(row + direction.y)))
				...
				if i < width - 3:
					if all_pieces[i + 1][j] != null and all_pieces[i + 2][j] != null and all_pieces[i + 3][j] != null:
						if all_pieces[i + 1][j].color == current_color and all_pieces[i + 2][j].color == current_color and all_pieces[i + 3][j].color == current_color and not all_pieces[i][j].matched and not all_pieces[i + 1][j].matched and (not all_pieces[i + 2][j].matched) and (not all_pieces[i + 3][j].matched):
							...
							if all_pieces[i + 3][j].type == "Row":
								eraseRow(i + 3, j)
				if (i < width - 2 and all_pieces[i + 2][j] != null and all_pieces[i + 1][j] != null and all_pieces[i + 2][j].color == current_color and all_pieces[i + 1][j].color == current_color and not all_pieces[i][j].matched and not all_pieces[i + 1][j].matched and (not all_pieces[i + 2][j].matched)):
					all_pieces[i + 2][j].matched = true
					all_pieces[i + 2][j].dim()
					all_pieces[i][j].matched = true
					all_pieces[i][j].dim()
				...
				if j <= height - 4:
					if all_pieces[i][j + 1] != null and all_pieces[i][j + 2] != null and all_pieces[i][j + 3] != null:
						if all_pieces[i][j + 1].color == current_color and all_pieces[i][j + 2].color == current_color and all_pieces[i][j + 3].color == current_color and not all_pieces[i][j].matched and not all_pieces[i][j + 1].matched and not all_pieces[i][j + 2].matched and not all_pieces[i][j + 3].matched:
							...
							if all_pieces[i][j + 3].type == "Row":
								eraseRow(i, j + 3)
				if (j < height - 2 and all_pieces[i][j + 2] != null and all_pieces[i][j + 1] != null and all_pieces[i][j + 2].color == current_color and all_pieces[i][j + 1].color == current_color and not all_pieces[i][j].matched and not all_pieces[i][j + 1].matched and not all_pieces[i][j + 2].matched):
